...
ui <- fluidPage(
    ...
    server <- function(input, output) {
        ...
        output$table <- renderDataTable(
           datatable({
             data <- mydata %>% select(region_x,annee,emissions_totale,variation_temperature,population_totale) %>% filter(region_x %in% input$region, annee == input$year) %>% group_by(region_x)
           }))
